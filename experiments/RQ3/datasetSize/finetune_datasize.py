...
def parse_args():
    ...
    if not args.log_file_name:
        args.log_file_name = f"log_datasize_{args.data_ratio}"
...
def main():
    ...
    file_prefix = "fine-tune-comments" if args.comments else "fine-tune-NOcomments"
    ...
    log_file_path = os.path.join(args.output_dir, args.model_name, file_prefix, f"{args.log_file_name}_{date_string}.txt")
    ...
    for epoch in range(args.num_epochs):
        ...
        if val_loss < best_validation_loss:
            ...
            best_model_path = os.path.join(args.output_dir, args.model_name, file_prefix, f"best_model_{date_string}")
            os.makedirs(best_model_path, exist_ok=True)
            model.save_pretrained(best_model_path)
            tokenizer.save_pretrained(best_model_path)

        # Save model for each epoch
        save_path = os.path.join(args.output_dir, args.model_name, file_prefix, f"epoch_{epoch + 1}")
