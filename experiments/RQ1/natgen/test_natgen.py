...
def main():
    ...
    content_write += f"test_model: {args.test_model}\n"  # 添加这行
    ...
    # 修正模型加载路径
    full_model_path = os.path.join(args.model_path, args.test_model)
    tokenizer = AutoTokenizer.from_pretrained(full_model_path)
    model = AutoModelForSequenceClassification.from_pretrained(full_model_path, num_labels=2)
    set_seed(args.seed)
    ...
    test_accuracy, test_f1, test_precision, test_recall, test_auc, test_report = evaluate_model(model, test_dataloader, device)

    test_res = "=" * 20 + "Test_res" + "=" * 20 + "\n"
    test_res += f"Accuracy:  {test_accuracy:.4f}\n"
    test_res += f"Precision: {test_precision:.4f}\n"
    test_res += f"Recall:    {test_recall:.4f}\n"
    test_res += f"F1 Score:  {test_f1:.4f}\n"
    test_res += f"AUC:       {test_auc:.4f}\n"
    test_res += f"Classification Report:\n{test_report}\n"
    test_res += "=" * 50 + "\n"
    print(test_res)
